...
class RightAdsApp {
    constructor() {
        ...
        this.nodes = {
            ...
            navbar: document.querySelector('.apple-navbar'),
...
const video = document.getElementById('RightAds-video');

// Intersection Observer (Video trigger karne ke liye)
const observer = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      // 1. Mute set karein taaki browser autoplay block na kare
      video.muted = true; 
      ...
      // 3. Screen se bahar jaane par pause karein
      video.pause();
    }
  });
}, { threshold: 0.5 }); // Jab 50% video screen par dikhe tab trigger ho

observer.observe(video);


function sendWhatsAppLead() {
    // 1. Checkboxes ka status check karein
    const services = [
        { id: 'webdev', name: 'Website Development' },
        { id: 'seo', name: 'Premium SEO' },
        { id: 'ads', name: 'Google & Social Ads' },
        { id: 'branding', name: 'Brand Kit' },
    ...
    // 2. Sliders ki values uthayein
    const pages = document.getElementById('pages-slider').value;
    ...
    let message = `*New Proposal Request*%0A%0A`;
    message += `*Selected Services:*%0A- ${selectedServices.join('%0A- ')}%0A%0A`;
    message += `*Customizations:*%0A`;
    message += `Pages: ${pages}%0A`;
    message += `Monthly Ads Budget: ₹${budget}%0A%0A`;
    message += `*Total Estimate:* ${total}%0A%0A`;
    message += `Please confirm these details.`;

    // 4. WhatsApp par bhejein
    const phoneNumber = "918377072990"; // Apna number
...
////
const navLinks = document.querySelectorAll(".apple-menu li a");
...
const rotateX = (rect.height / 2 - y) / 1.2;
...
mobileTheme.innerHTML="☀️ Light Mode";
...
mobileTheme.innerHTML="🌙 Dark Mode";
